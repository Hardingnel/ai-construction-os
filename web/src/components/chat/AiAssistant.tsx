...
import { Send, X, Bot, User, MessageSquare, Sparkles, Loader2 } from 'lucide-react';
...
export function AiAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'assistant', content: "Hello! I'm your AI Construction Assistant. I can help you design buildings, analyze sites, generate BOQs, and more. What would you like to create?", timestamp: new Date() },
  ]);
  ...
  return (
    <>
      ...
      <div className={cn('fixed bottom-0 right-0 z-40 h-[calc(100vh-2.5rem)] w-96 border-l bg-card shadow-2xl transition-transform duration-300 ease-in-out flex flex-col', isOpen ? 'translate-x-0' : 'translate-x-full')}>
        <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-blue-600/10 to-purple-600/10">
          ...
          <button onClick={() => setIsOpen(false)} className="h-8 w-8 rounded-lg hover:bg-accent flex items-center justify-center transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="p-4 border-t">
          <div className="flex gap-2">
            <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Describe your building project..." className="flex-1" />
            <Button onClick={handleSend} disabled={isLoading || !input.trim()} size="icon" className="shrink-0 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700">
